...
\subsubsection{迫敛性}
\begin{theorem}
	设 $\{x_n\}, \{y_n\}$ 是两个收敛数列, 并且 $\lim\limits_{n\to\infty} x_n = A,  \lim\limits_{n\to\infty} y_n = B$.如果 $A < B$, 就可以求出序号 $N \in \N$, 使得对于任何 $n > N$, 不等式 $x_n < y_n$ 成立.
	
	设数列 $\{x_n\}, \{y_n\}, \{z_n\}$ 满足以下条件, 对于任何 $n > N \in \N$, 关系式 $x_n \le y_n \le z_n$ 成立.如果数列 $\{x_n\}, \{z_n\}$ 收敛于同一个极限, 则数列 $\{y_n\}$ 页收敛于该极限. 
\end{theorem}
\begin{proof}
...
如果 $x_1, x_2, \cdots, x_n, \cdots$ 是某数列, 而 $n_1 < n_2 < \cdots < n_k < \cdots$ 是严格单调递增自然数列, 则数列 $x_{n_1}, x_{n_2}, \cdots, x_{n_k}, \cdots$
