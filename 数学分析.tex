...
\subsection{元组}
设 $m$ 是一个正整数, 对于给定的集合 $X$, $X$ 中元素的一个 $m$ 元组定义为
\[
	x:\{1,\cdots,m\}\to X.
\]
若 $x$ 是一个 $m$ 元组, 则往往把 $x$ 在 $i$ 项的值记为 $x_i$, 并称之为 $x$ 的第 $i$ 个坐标, 函数本身写为
\[
	(x_1,\cdots,x_m).
\] 
...
\[
	\prod_{i=1}^m A_i 或者 A_1 \times \cdots \times A_m,
\]
...
其实极限的思想很早就在数学家们的心中产生, 无论是刘徽的割圆术还是芝诺的阿基里斯追龟悖论, 都是极限思想的体现.
