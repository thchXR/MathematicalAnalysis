...
离散数学是现代数学的重要分支，也是计算机科学的核心基石，主要研究不连续的、离散的数学结构和对象（如整数、命题和图）。
...
\subsubsection{合取}
设 $p,q$ 为命题， $p,q$ 的合取为一个命题，记为 $p\land q$ ，即 $p,q$ 均为 T 时 $p\land q$ 取 T ，否则 $p\land q$ 取 F

\subsubsection{析取}
设 $p,q$ 为命题， $p,q$ 的析取为一个命题，记为 $p\lor q$ ，即 $p,q$ 均为 F 时 $p\lor q$ 取 F ，否则 $p\lor q$ 取 T
...
\subsection{策梅洛-弗伦克尔公理系统 Zermelo-Fraenkel}
\ITEMS
	\item 外延公理
